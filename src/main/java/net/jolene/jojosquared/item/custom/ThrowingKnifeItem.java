...
public class ThrowingKnifeItem extends Item {
    ...
    public ActionResult use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient) {
            ThrowingKnifeProjectileEntity throwing_knife = new ThrowingKnifeProjectileEntity(world, user);
            throwing_knife.setVelocity(user, user.getPitch(), user.getYaw(), 0.0f, 1.5f, 0f);
            world.spawnEntity(throwing_knife);
        }
